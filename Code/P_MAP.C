...
//           if (tmthing->z == tmfloorz)
           ...
           {
             if(((tmthing->eflags & MF_JUMPED && !(tmthing->eflags & MF_ONGROUND)) || (tmthing->eflags & MF_JUSTJUMPED && !(tmthing->eflags & MF_ONGROUND)) || (tmthing->player->powers[pw_invulnerability] && !(tmthing->eflags & MF_ONGROUND))))
//              if(((tmthing->state->nextstate==S_PLAY_ATK3 || tmthing->state->nextstate==S_PLAY_ATK4 || tmthing->state->nextstate==S_PLAY_ATK5 || tmthing->state->nextstate==S_PLAY_ATK6) && tmthing->eflags & ~MF_ONGROUND) || tmthing->player->powers[pw_invulnerability])
 ...
 //                  tmthing->player->mo->momy = 0;
            ...
            else if(((tmthing->eflags & MF_JUMPED) || (tmthing->eflags & MF_JUSTJUMPED) || tmthing->player->powers[pw_invulnerability])  && tmthing->type==MT_PLAYER)
           ...
           if (tmthing->player && tmthing->z >= tmfloorz)
               ...
               switch(thing->type) {
                 ...
                 tmthing->player->mo->momz = JUMPGRAVITY;
                   ...
                   // Restoring fan animation SSNTails 03-10-2024
                   if (!(tmthing->player->mo->state == &states[S_PLAY_FAN1]
                       || tmthing->player->mo->state == &states[S_PLAY_FAN2]
                       || tmthing->player->mo->state == &states[S_PLAY_FAN3]
                       || tmthing->player->mo->state == &states[S_PLAY_FAN4]
                       || tmthing->player->mo->state == &states[S_PLAY_FAN5]
                       || tmthing->player->mo->state == &states[S_PLAY_FAN6]
                       || tmthing->player->mo->state == &states[S_PLAY_FAN7]
                       || tmthing->player->mo->state == &states[S_PLAY_FAN8]))
                        P_SetMobjState (tmthing->player->mo, S_PLAY_FAN1);
                 break;
                 default:
                 break;
             }
